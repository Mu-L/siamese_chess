extends "res://src/level/outside.gd"

var battle_finished:bool = false

func _ready() -> void:
	super._ready()
	battle_finished = Progress.has_key("outside_7_battle") && Progress.get_value("outside_7_battle")
	if !battle_finished:
		var table:Array = [
			{
				"actor": load("res://scene/actor/enemy_cheshire.tscn").instantiate(),
				"pos": 0x61,
				"piece": ord("K")
			},
			{
				"actor": load("res://scene/actor/piece_bishop_white.tscn").instantiate().set_larger_scale(),
				"pos": 0x66,
				"piece": ord("B")
			},
			{
				"actor": load("res://scene/actor/piece_bishop_white.tscn").instantiate().set_larger_scale(),
				"pos": 0x11,
				"piece": ord("B")
			},
			{
				"actor": load("res://scene/actor/piece_knight_white.tscn").instantiate().set_larger_scale(),
				"pos": 0x31,
				"piece": ord("N")
			},
			{
				"actor": load("res://scene/actor/piece_knight_white.tscn").instantiate().set_larger_scale(),
				"pos": 0x52,
				"piece": ord("N")
			},
			{
				"actor": load("res://scene/actor/piece_rook_white.tscn").instantiate().set_larger_scale(),
				"pos": 0x33,
				"piece": ord("R")
			}
		]
		for iter:Dictionary in table:
			var actor:Actor = iter["actor"]
			var piece_pos:int = iter["pos"]
			actor.position = $chessboard.name_to_vector3(Chess.to_position_name(piece_pos))
			$chessboard.state.add_piece(piece_pos, iter["piece"])
			$chessboard.add_piece_instance(actor, piece_pos)
	connect("level_state_changed", on_level_state_changed)

func on_level_state_changed(state:String) -> void:
	if state == "black_win":
		battle_finished = true
		Progress.set_value("outside_7_battle", true)
